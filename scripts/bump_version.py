...
def set_version_in_file(filepath, regex, new_version_str):
    ...
    if current_version_match.group(2) == new_version_str:
        print(
            f"Version in {filepath} is already {new_version_str}. No change made to file.",
            file=sys.stderr,
        )
    ...
    if (
        content == new_content
    ):  # Should not happen if previous check passed and new_version is different
        raise RuntimeError(
            f"Failed to update version in {filepath}. Content might be malformed or regex issue."
        )
...
def increment_version_part(current_version, part="patch"):
    ...
    if part == "major":
        ...
        raise ValueError("Invalid version part. Choose 'major', 'minor', or 'patch'.")
    return f"{major}.{minor}.{patch}"


def main():
    parser = argparse.ArgumentParser(
        description="Manage project version in src/taskgist/__init__.py."
    )
    ...
    # Action: current
    subparsers.add_parser("current", help="Print current version to stdout.")

    # Action: bump
    parser_bump = subparsers.add_parser(
        "bump",
        help="Bump version part (major, minor, patch) and print new version to stdout.",
    )
    parser_bump.add_argument(
        ...
        default="patch",
        nargs="?",
        help="Part to bump (default: patch).",
    )

    # Action: set
    parser_set = subparsers.add_parser(
        "set", help="Set a specific version and print it to stdout."
    )
    ...
    if args.action == "bump":
        ...
        print(
            f"Current version: {current_v}. Bumping '{args.part}' to: {new_v}",
            file=sys.stderr,
        )
    ...
    if new_v:
        if new_v != current_v:
            ...
            print(
                f"Version is already {new_v}. No file update needed.", file=sys.stderr
            )
